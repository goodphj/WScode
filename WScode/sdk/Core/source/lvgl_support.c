...
{
#if (LV_USE_GESTURE_RECOGNITION == 1)
    static touch_point_t touchPoints[FT5406_RT_MAX_TOUCHES];
    
    for (int i = 0; i < FT5406_RT_MAX_TOUCHES; i++) {
        touches[i].id = i;
        touches[i].point.x = 0;
        touches[i].point.y = 0;
    ...
    if (kStatus_Success == FT5406_RT_GetMultiTouch(&touch_handle, &touch_cnt, touchPoints))
    {
        for (int i = 0; i < FT5406_RT_MAX_TOUCHES; i++) {

            touch_event_t event = touchPoints[i].TOUCH_EVENT;
            switch (event)
            {
            case kTouch_Down:
            case kTouch_Contact:
                touches[i].id = touchPoints[i].TOUCH_ID;
                touches[i].point.x = touchPoints[i].TOUCH_X;
                touches[i].point.y = touchPoints[i].TOUCH_Y;
                touches[i].state = LV_INDEV_STATE_PRESSED;
                break;

            case kTouch_Up:
            case kTouch_Reserved:
                touches[i].id = i;
                touches[i].state = LV_INDEV_STATE_RELEASED;
                break;
    ...
    lv_indev_gesture_recognizers_update(drv, touches, FT5406_RT_MAX_TOUCHES);
